...
-- NOTE: This migration was applied manually before GitHub Actions was configured.
-- This file ensures migration history is consistent.
...
CREATE POLICY IF NOT EXISTS "Users can update their household recipes" ON recipes
  FOR UPDATE USING (
    household_id IN (SELECT id FROM households WHERE created_by = auth.uid())
  );

CREATE POLICY IF NOT EXISTS "Users can delete their household recipes" ON recipes
  FOR DELETE USING (
    household_id IN (SELECT id FROM households WHERE created_by = auth.uid())
  );
...
CREATE POLICY IF NOT EXISTS "Users can manage ingredients of their recipes" ON recipe_ingredients
  FOR ALL USING (
    recipe_id IN (
...
CREATE POLICY IF NOT EXISTS "Users can manage their household meal plans" ON meal_plan_items
  FOR ALL USING (
    household_id IN (SELECT id FROM households WHERE created_by = auth.uid())
  );
...
CREATE POLICY IF NOT EXISTS "Users can manage their household weekly focus" ON weekly_focus
  FOR ALL USING (
    household_id IN (SELECT id FROM households WHERE created_by = auth.uid())
  );
...
CREATE POLICY IF NOT EXISTS "Users can manage their household shopping list" ON shopping_list_items
  FOR ALL USING (
    household_id IN (SELECT id FROM households WHERE created_by = auth.uid())
  );
